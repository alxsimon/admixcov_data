...
rng = np.random.default_rng(826383)
...
ds_full['mask_cohorts'] = (
    ['cohorts', 'samples'],
    [[i in gs for i in ds_full.sample_group.values] for gs in time_periods_grouping.values()]
)
...
kept_loci = (
    np.all((ds_full.variant_count_nonmiss.values > 2), axis=0) 
    & np.all((ds_full.variant_count_nonmiss_ref.values > 2), axis=0)
)
...
ascert = pd.concat((
        pd.read_csv(
            f"results/Papac2021/ascertainment_info/Rohland2022_supp_chr{chr}.txt",
            sep='\t',
            header=None,
            usecols=[0, 1, 23],
